...
/// <summary>Append direct float32 GX triangles and a constant grey material. Original offsets remain stable.</summary>
public static class ModelArchiveWriter
{
    public static byte[] Write(ArchiveLayout source, EditableModel target, MeshData mesh)
    {
        ...
        var attributes = new byte[72];
        for (int i = 0; i < 2; i++)
        {
            Put(attributes, i * 24, 9 + i); // GX_VA_POS / GX_VA_NRM
            Put(attributes, i * 24 + 4, 1); // GX_DIRECT
            Put(attributes, i * 24 + 8, i == 0 ? 1 : 0); // XYZ / NRM_XYZ
            Put(attributes, i * 24 + 12, 4); // GX_F32
            Short(attributes, i * 24 + 18, 12);
        }
        Put(attributes, 48, 255); // GX_VA_NULL
        int attrs = Append(attributes);
        int length = checked((3 + mesh.Positions.Length * 24 + 31) / 32 * 32);
        ...
        byte[] mobj = new byte[24]; Put(mobj, 4, 1); Put(mobj, 12, material); // RENDER_CONSTANT, no textures/lighting
        int mobjOffset = Append(mobj);
        byte[] payload = data.ToArray(); int pOffset = target.PobjOffset;
        // Replacement triangles use Blender's winding and cross-product normals.
        // Do not inherit a source mesh's front-face (or both-face) culling: it
        // makes newly authored outward-facing geometry appear inside out.
        int flags = new ArchiveDataReader(source).UShort(pOffset + 12);
        Short(payload, pOffset + 12, (flags & ~0xC000) | 0x4000); // POBJ_CULLBACK
        ...
        foreach (int field in new[] { pOffset + 8, pOffset + 16, target.DobjOffset + 8, mobjOffset + 12 })
        ...
        Verify(parsed, target, mesh);
        return output;
    }

    public static void Verify(ArchiveLayout archive, EditableModel target, MeshData expected)
    {
        ...
        Require((r.UShort(target.PobjOffset + 12) & 0xC000) == 0x4000,
            "MODEL_CULL_MISMATCH", "Replacement model must use back-face culling.");
