...
public static class TexturePreview
{
    public static MaterialPreview Extract(ArchiveLayout archive, ModelMaterial material, string directory)
    {
        ...
        float[] diffuse = color.HasValue ? Enumerable.Range(0, 3).Select(i => r.Byte(color.Value + 4 + i) / 255f).Append(1f).ToArray()
            : [0.45f, 0.45f, 0.45f, 1];
        float[] ambient = color.HasValue ? Enumerable.Range(0, 3).Select(i => r.Byte(color.Value + i) / 255f).Append(1f).ToArray()
            : diffuse;
        ...
        MaterialPreview Preview(PreviewTexture[]? textures, string? warning = null) => new(diffuse, textures?.FirstOrDefault(),
            ...
            specular, shininess, ambient, textures);
